...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    ...
    try {
      ...
      if (isValid) {
        const { reciver, sender, sender_name, sender_photo, room_id, time } = req.body;

        // Insert original message into Supabase
        const { data: insertedMessage, error } = await supabase.rpc('create_fn_livechat_reminder', {
          new_thread_room_id: room_id,
          new_content: `Hey ${reciver} <br/>
          Legalnowy ingin mengingatkan waktu livechat kamu tersisah ${time} lagi nih.`,
          new_role: sender,
          new_type_chat: 'text',
        ...
        res.status(200).json({ message: `Waktu tersisah ${time} lagi` });
      } else {
        return res.status(401).json({ message: 'Unauthorized' });
      }
    } catch (error) {
